...
import {
  ...
  PrestadorTuristico,
  Ruta,
} from 'src/app/common/place.interface';
...
export class PrestadoresService {
  ...
  agregarPrestador(prestador: any, files: any, portadaFile: any): Promise<any> {
    ...
    if (!(portadaFile === undefined)) {
      ...
      const imgRef = ref(
        this.storage,
        `prestadoresStorage/${prestador.name}/ImagenPrincipal/${portadaFile.name}`
      );
    ...
    if (!(files.length === 0)) {
      ...
      for (let file of files) {
        ...
        const imgRef = ref(
          this.storage,
          `prestadoresStorage/${prestador.name}/${file.name}`
        );
  ...
  borrarImagenesPrestador(prestador: any) {
    ...
    if (!(pathImgPrincipal === '')) {
      ...
      deleteObject(refImgPrincipal)
        .then(() => {
          console.log('Se ha borrado la img Principal de: ', prestador.name);
        })
        .catch((error) => {
